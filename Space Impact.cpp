...
void iDraw()
{
    ...
    if (gameMode==4) ///high score - DONE!
    {
        ...
        for (i=0; i<5; i++)
        {
            ...
            iText(460, yyy, tmpo, GLUT_BITMAP_TIMES_ROMAN_24);
            iText(470, yyy, ". ", GLUT_BITMAP_TIMES_ROMAN_24);
            iText(500, yyy, scores[i].plr, GLUT_BITMAP_TIMES_ROMAN_24);
            iText(770, yyy, tmpn, GLUT_BITMAP_TIMES_ROMAN_24);
            yyy -= 50;
        }
    }
...
void iSpecialKeyboard(unsigned char key) ///to move my spaceship
{
    if (gameMode==1)
    {
        ...
        {
            if (spaceship_pos_x+10<1250 && spaceship_pos_x+20<boss.x) spaceship_pos_x+=10;
        }
...
void enemyMove() ///to move the enemy; ok
{
    if (gameMode==1)
    {
        for (j=0; j<enemyNumber; j++)
        {
            if (enemyArray[j].alive==1) enemyArray[j].x -= 4;
            if (enemyArray[j].x+50 < 0) enemyArray[j].alive = 0;
        }
    }
}
...
void enemyBeamMove() ///to move beams of each individual enemy
{
    if (gameMode==1)
    {
        for (j=0; j<enemyNumber; j++)
        {
            for (i=0; i<MAX_ENEMY_BEAM; i++)
            {
                if (enemyBeamArray[j][i].is_shoot==1) enemyBeamArray[j][i].x -= 25;
                if (enemyBeamArray[j][i].x+10 < 0) enemyBeamArray[j][i].is_shoot = 0;
            }
        }
    }
}
    ...
    if (gameMode==1)
    {
        if (boss.alive==1)
        {
            for (j=0; j<bossBeamNumber; j++)
            {
                ...
                if (bossBeamArray[j].x+10<0) bossBeamArray[j].is_shoot=0;
            }
        }
    }
}
